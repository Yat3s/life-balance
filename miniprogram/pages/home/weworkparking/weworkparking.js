...
Page({
  ...
  onLoad: function (options) {
    let { bookingToday } = this.data;
    const now = new Date();
    if (
      now.getHours() >= bookingTodayStart &&
      now.getHours() < bookingTodayEnd
    ) {
      bookingToday = true;
    }

    const bookingTomorrowDate =
      now.getHours() < bookingTodayStart ? new Date() : new Date().addDays(1);
    const bookingTomorrowDateStr =
      (bookingTomorrowDate.getDate() == new Date().getDate()
        ? "今天 "
        : "明天 ") + bookingTomorrowDate.mmdd();
    const bookingTomorrowAvailableDateStr = `${bookingTomorrowDate
      .addDays(-1)
      .mmdd()} ${bookingTodayEnd}:00 - ${bookingTomorrowDate.mmdd()} ${bookingTodayStart}:00 `;
    ...
    fetchUserInfo().then((userInfo) => {
      ...
      getAppConfig().then((config) => {
        ...
        for (const org of allowedOrgs) {
          if (
            org.name.toLowerCase() === userInfo.org.toLowerCase() &&
            userInfo.alias &&
            userInfo.vehicleId
          ) {
            userAllowed = true;
  ...
  bookingCountdown() {
    ...
    const bookingTomorrowEndDate = new Date()
      .addDays(new Date().getHours() < bookingTodayEnd ? 0 : 1)
      .setHours(bookingTodayStart);
    const countDownDate = bookingToday
      ? new Date().setHours(bookingTodayEnd, 0, 0)
      : bookingTomorrowEndDate;
    const x = setInterval(() => {
  ...
  fetchBooking() {
    const { userInfo, allowedOrgs, bookingToday } = this.data;
    let joined = false;
    ...
    fetchWeworkParkingBooking().then((bookings) => {
      const booking =
        new Date().getHours() < bookingTodayEnd ? bookings[1] : bookings[0];
      const commonPool = {
        participants: [],
        maxParticipant: 0,
      };
      ...
      for (const participant of booking.participants) {
        ...
        for (const org of allowedOrgs) {
          if (participant.org.toLowerCase() === org.name.toLowerCase()) {
            ...
            const participantJoinedWithinTodayBooking =
              participantJoinedDate.getDate() == new Date().getDate() &&
              participantJoinedDate.getHours() >= bookingTodayStart &&
              participantJoinedDate.getHours() < bookingTodayEnd;

            if (
              bookingToday &&
              (org.participants.length === org.maxParticipant ||
                participantJoinedWithinTodayBooking)
            ) {
      ...
      for (const org of allowedOrgs) {
        commonPool.maxParticipant +=
          org.maxParticipant - org.participants.length;
      }
  ...
  onBookingClick() {
    ...
    if (
      bookingToday &&
      commonPool.maxParticipant <= commonPool.participants.length
    ) {
      wx.showToast({
        icon: "error",
        title: "剩余车位为 0",
