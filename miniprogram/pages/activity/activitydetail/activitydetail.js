...
Page({
  ...
  calcDistance(latitude, longitude) {
    ...
    if (activity.location && activity.location.latitude) {
      const distance = util.calcDistance(
      ...
      activityRepo
        ...
        .then((activity) => {
          ...
          for (const participant of activity.participants) {
            ...
            participant.joinedAtStr =
              joinedAtDate.dateStr() + " " + joinedAtDate.hhmm();
          }

          const participantMaleProgress =
            participantMale == 0
              ? 0
              : Math.max(
                  20,
                  parseInt(
                    (participantMale / activity.maxParticipantMale) * 100
                  )
                );
          const participantFemaleProgress =
            participantFemale == 0
              ? 0
              : Math.max(
                  20,
                  parseInt(
                    (participantFemale / activity.maxParticipantFemale) * 100
                  )
                );
          const maxParticipantMalePercent = parseInt(
            (activity.maxParticipantMale / activity.maxParticipant) * 100
          );
          const maxParticipantFemalePercent = parseInt(
            (activity.maxParticipantFemale / activity.maxParticipant) * 100
          );
  ...
  showQrcodeModal() {
    const { joined, activity } = this.data;
  ...
  signupActivity(activitySnapshot, userInfo) {
    ...
    this.fetchActivity(activitySnapshot._id)
      .then((activity) => {
        const {
          participantMale,
          participantMaleProgress,
        ...
        activityRepo
          .signupActivity(activity._id, userInfo)
          .then((data) => {
  ...
  onSignupActivity(e) {
    const { activity, joined, isOrganizer, userInfo, ended } = this.data;
  ...
  onClickLike(e) {
    const { liked, activity } = this.data;
    if (liked) {
